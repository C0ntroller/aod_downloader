...
class AoDDownloader:
    ...
    def get_all_animes(self):
        ...
        for box in all_boxes:
            all_anime.append({'title': box.div.h3.string, 'link': box.div.find(
                class_='animebox-link').a['href'], 'movie': ('film' in box.div.find(class_='animebox-link').a.string.lower())})
    ...
    def get_all_episodes(self, sublink: str, isMovie: bool):
        ...
        if isMovie:
            title = ''.join(list(self.browser.find(
            ...
            for box in all_boxes:
                title = list(map(lambda x: str(x), box.find(
                    class_='episodebox-title').contents))
                title = (' '.join(title)).replace('<br/>', '-')
                playlist_sub = box.find(
                    attrs={'data-lang': 'jap'}, class_='streamstarter_html5')
                if playlist_sub:
                    playlist_sub = playlist_sub['data-playlist']
                playlist_dub = box.find(
                    attrs={'data-lang': 'ger'}, class_='streamstarter_html5')
                if playlist_dub:
                    playlist_dub = playlist_dub['data-playlist']

                all_episodes.append(
                    {'title': title, 'playlist_sub': playlist_sub, 'playlist_dub': playlist_dub})
    ...
    def get_multi_episodes_playlists(self, playlist_link, e_range):
        all_file_links = self._get_all_episode_playlist(
            playlist_link)['playlist'][0:e_range+1]
    ...
    def download_episode(self, playlist: str, title: str, usesDub: bool = False, dir_name: str = ''):
        title = title.replace('ä', 'ae').replace('ö', 'oe').replace('ü', 'ue').replace(
            'ß', 'ss').replace('Ä', 'Ae').replace('Ö', 'Oe').replace('Ü', 'Ue')
...
def download_episodes(e_list: list, e_select: str, useDub: bool, dir_name: str = ''):
    if re.match('^[0-9][0-9]*-[0-9][0-9]*$', e_select):
        ...
        if useDub:
            file_link = aod.get_single_episode_playlists(
                e_list[int(e_select)-1]['playlist_dub'])
        else:
            file_link = aod.get_single_episode_playlists(
                e_list[int(e_select)-1]['playlist_sub'])
        aod.download_episode(file_link, e_list[int(
            e_select)-1]['title'], useDub, dir_name)
